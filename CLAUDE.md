...
- ⚠️ **mission-control 반영 재개(2026-09-08)** — `lib/mission_control_sync.py`/
  `lib/dashboard.py`의 플랫폼 필터가 카드뉴스 단일 트랙 시절 기준(영상 타입
  전부 제외)에 머물러 있어 mission-control 업로드 탭에서 세 플랫폼이 안 떴다.
  **유튜브 쇼츠·네이버 클립·페이스북만** 다시 노출하도록 수정(인스타 릴스·
  틱톡은 성장 우선 판단으로 계속 보류) — mission-control `page.tsx`의
  카드뉴스/영상 분류 로직도 같이 고침(네이버 블로그가 `type:"text"`라 영상
  탭으로 잘못 묶이던 문제).
