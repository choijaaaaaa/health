...
- 의도적으로 다른 목소리를 쓸 일이 있으면 호출부에서 `voice_name`을 명시할 것.
...
- 위치는 `CTA_CX/CTA_CY` 상수 — 칠판 나무 선반 위(y≈1545)다. 그 아래 검은 여백
  (1760~1920)이 광고 배너 자리고, 좌측 x<260은 "급식 당번" 명패, 우측 x>760은
  원형 사진이 이미 쓰고 있어 그 사이만 비어 있다. **레이아웃을 바꾸면 이 값도 같이
  확인할 것.**
...
- ⚠️ **CTA가 붙으면 비디오를 재인코딩해야 한다**(`-c:v copy` 불가). 조립이 그만큼
  느려지는 건 정상이다.
- 기존 108편은 2026-09-14에 소급 적용했다. **CTA 없는 원본은 `<name>.pre_cta.mp4`로
  같은 폴더에 남아 있다** — 디자인을 다시 손볼 땐 이 원본에서 다시 입히면 되고,
  현재본에 또 얹지 말 것(CTA가 두 겹으로 쌓인다).
