...
포르투갈어·러시아어 6개로 확정**. 그 외 언어(대만어·프랑스어·독일어·베트남어·
아랍어·벵골어·터키어·태국어·인도네시아어·힌디어)는 data/output 콘텐츠 전부
삭제, `data/global_research_rules.md`·`typecast_voices_global.json`·
`global_channels.json`도 5개 언어(영/일/스/포/러)만 남기고 정리함. 새
topic·언어는 이 6개 범위 안에서만 진행할 것 — 확장이 다시 필요해지면 git
히스토리에서 복원.

- **번역 금지 — 언어마다 독립 리서치**(원인·해결책·이미지 전부 그 지역
  원문 자료 기준). topic(다룰 주제) 자체는 공통, 실질 콘텐츠만 독립.
- ⚠️ **향후 아마존/알리익스프레스 제휴 예정, 아직 미착수** — 지역별 표시광고
  법규·아마존/알리 프로그램 가용성·필요 문구까지 리서치 완료해뒀음(archive
  "글로벌 제휴 준비 리서치" 절). 착수 시 그 절부터 참고할 것 — 특히 **러시아는
  아마존 자체가 불가**(2022년부터 러시아 배송 중단, 복구 안 됨)라 AliExpress.ru만
  가능하고, 채널은 유지하되 모네타이즈는 나머지 4개 언어보다 뒤 순서로 미루기로
  확정됨.
...
- ⚠️ **밀린 backlog를 몰아 올릴 때 — `uploadLimitExceeded`(계정당 하루 실제
  ...
  고정된 안전 숫자를 미리 정해두지 말 것** — 한도 초과는 topic 하나만
  실패하고 에러 없이 넘어가는 구조(`upload_short`가 topic별 try/except로
  격리돼 있어 한 채널이 한도에 걸려도 다른 채널·다음 topic엔 영향 없음)라
  넉넉하게 요청해도 안전함. 남은 backlog 재개 시 그냥 "채널별로 이어서
  올려줘"로 진행하고, 한도에 걸린 topic은 다음날 이후로 자연히 넘기면 됨.
