...
- ⚠️ **소재 선정(2026-08-10)** — "다음 topic 해줘"처럼 소재가 정해지지 않은
  ...
  1. **비-건강 일반 트렌드부터 WebSearch로 훑기** — "오늘 실시간 이슈"/
     "이번 주 화제" 등으로 검색해서 지금 뜨는 일반 트렌드(음식·상품·이슈,
     건강 카테고리 무관)를 확인. 건강과 연결될 만한 게 있으면 그 자리에서
     리서치해서 근거(기전·수치)를 확인하고 그 각도로 진행 — **억지 연결
     금지, 근거가 안 나오면 그냥 폐기하고 다음 단계로**(자동 판정기를 따로
     만들지 않고 세션이 매번 직접 판단하는 이유는 `data/seasonal_topics.md`
     상단 절 참고).
