...
- ⚠️ **TTS는 언어 상관없이(한국어 포함) topic당 1회만 호출한다** — 길이가
  마음에 안 들어도(45초 초과, 1분 넘음, 톤이 어색함 등 어떤 이유든) narration.txt를
  고쳐서 재생성하지 않는다(2026-08-05, "돈 존나 나간다" — TTS는 글자수
  과금이라 "뽑고→마음에 안 들면 재작성→다시 뽑고" 반복이 API 호출 배수로
  이어짐). 원래 이 반전은 "비한국어 topic 45초 하드 캡"(archive 참고) 규칙을
  뒤집으면서 나온 거라 문구가 비한국어로 좁게 박혀있었는데, Typecast 과금은
  언어 무관하게 똑같이 적용되므로 한국어 topic도 똑같이 1회 호출로 끝낼
  것(2026-08-05 재확인 — 스코프 좁은 문구 때문에 헷갈릴 수 있어 명시적으로
  확장). 대신 `lib/typecast_tts.py`의 `estimate_duration(text, lang)`으로
