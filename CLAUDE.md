...
- **필수 플래그**(한국어 topic, 빠짐없이):
  ...
  - `suppress_product_block: true` — 유튜브 쇼츠·인스타그램 릴스·틱톡·
    페이스북엔 계속 건다(위 "[광고]" 항목 참고). **네이버 클립은 2026-08-06부터
    안 건다**(`false` 또는 필드 자체를 생략) — 커넥트 재개. 네이버 블로그·
    티스토리·쓰레드·인스타 카드뉴스처럼 영상이 안 붙는 플랫폼엔 애초에
    걸지 말 것.
