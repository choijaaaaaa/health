...
- ⚠️ **블로그 제목(`platform_captions.json`의 `title` 필드)은 25~40자
  (2026-08-10)** — ⚠️ **`test_blog_title_length`는 실제로 존재하지 않음
  (2026-08-12 감사에서 발견 — 자동 검사된다는 이 문구 자체가 틀렸었음)**.
  실측: 86개 topic 중 58개(67%)가 25~40자 범위 밖(중앙값 47.5자, 최대
  85자) — 규칙 그대로 강제할지 실제 관행(긴 훅 문장형 제목)에 맞춰 범위를
  올릴지 사용자 판단 필요, 결정 전까지 테스트 추가 보류.
