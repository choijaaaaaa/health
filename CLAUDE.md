...
- ⚠️ **인스타그램 릴스용 안전 여백 영상 — 포맷 무관 전부 필수**: 칠판 포맷만의
  ...
  되는 호환성 문제**다 — 4개 템플릿(판서형/`before_after_transition`/
  `checklist`/`ranking_countdown`) 전부, ko/en/ja/es/pt(러시아어 제외, 아래
...
- ⚠️ **영상에 "[광고]" 표시 필요 — 미구현, 구현 전까지 기존 topic은 광고/링크
  ...
  - 아직 이 영상 오버레이가 구현 안 됨 — **구현되기 전까지 이미 만들어진(또는
    지금 만들고 있는) 한국어 topic은 커미션 관련 요소(링크·고지문구)를 전부
    뺀 순수 정보성 콘텐츠로 유지**한다: 모든 플랫폼에 `suppress_product_block:
    true`를 걸어 대시보드 JS(`applyProductLinks`)가 어떤 링크·고지문구도
    삽입하지 않게 하고, 캡션 본문에 정적으로 넣어둔 고지문구(있다면)도 뺀다.
  - **향후(오버레이 구현 후) topic**: 영상에 "[광고]" 플래그가 실제로 붙은
    topic만 캡션에 링크+광고 관련 코멘트를 붙이고, 플래그 없는 topic은 지금처럼
    순수 정보성으로 유지 — 오버레이 구현 시 이 플래그 기반 분기 로직도 같이 설계할 것.
- **필수 플래그**(한국어 topic, 빠짐없이):
  ...
  - `suppress_product_block: true` — 위 "[광고]" 항목 때문에 지금은 사실상
    한국어 topic 전체 플랫폼에 걸어야 함(구현 전까지)
