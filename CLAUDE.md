...
- **인스타그램 릴스용 안전 여백 영상 — 포맷 무관 전부 필수**(2026-08-05 명확화,
  "신규 포맷이여도 영상은 무조건 크롭해야 해 그 비율대로 넣어야 데스크탑에서
  인스타 업로드가 바로 가능해"): 칠판 포맷만의 문제가 아니라 **데스크탑에서
  인스타그램 업로드 자체가 이 비율이어야 바로 되는 호환성 문제**다 — 4개
  템플릿(판서형/`before_after_transition`/`checklist`/`ranking_countdown`)
  전부, ko/en/ja/es/pt(러시아어 제외, 위 "플랫폼 캡션" 절 참고) 전 언어에
  빠짐없이 만들어야 한다. `build_instagram_safe_video(source_path, out_path)`
  (기본 상하좌우 20% 여백, 실기기 테스트로 확정)로 `<원본파일명>_instagram.mp4`를
  같은 폴더에 만들어두면 `dashboard.py`가 자동으로 찾아서 인스타그램 릴스
  카드에만 연결한다(원본 `_shorts.mp4`는 유튜브 자동 업로드용으로 그대로 둠 —
  건드리지 않음). **영상을 새로 만들거나 재생성할 때마다 포맷 상관없이 항상
  같이 만들 것** — "칠판 포맷일 때만"으로 오해해서 신규 포맷 영상 다수에서
  누락된 사고가 있었음(2026-08-05, 47개 발견 후 일괄 보정).
...
- ⚠️ **"길이" 때문에 TTS를 재생성하는 것만 금지다 — "내용에 실제 오류가 있으면"
  고치고 재생성하는 건 당연히 허용이고 필요하다**(2026-08-05 재확정 — "문제가
  있으면 당연히 tts 고쳐야지 그거말고 45초 기준잡고 tts 다시 만들던거 그걸
  문제시했던거"라고 명확화. 한 번 "내용 수정 시 재생성도 금지"로 과잉 확대했다가
  바로 되돌림 — 헷갈리지 말 것). 기준:
  - **금지**: 길이가 마음에 안 들어서(45초 초과, 1분 넘음 등) 다시 뽑는 것.
    TTS를 이미 호출했는데 그냥 길다는 이유만으로 narration.txt를 손봐서
    재생성하지 않는다(2026-08-05, "돈 존나 나간다" — TTS는 글자수 과금이라
    "뽑고→길어서 재작성→다시 뽑고" 반복이 API 호출 배수로 이어짐). 대신
    `lib/typecast_tts.py`의 `estimate_duration(text, lang)`으로 **작성 전에**
    예상 길이를 가늠해서 애초에 적당한 분량으로 쓴다 — `data/tts_pacing.json`
    (언어별 초당 단어/글자수)은 `synthesize()` 호출마다 실제 결과로 자동
    갱신되는 경험 기반 누적 데이터라 topic이 쌓일수록 정확해진다(수동
    유지보수 불필요, 손대지 말 것).
  - **허용(권장)**: `content_review`/`--lang-check`가 실제 사실관계 오류·논리
    오류·과장 표현을 잡아냈으면 narration.txt를 고치고 TTS를 다시 호출한다 —
    오디오와 최종 텍스트가 어긋난 채로(=녹음된 내용에 실제 오류가 남은 채로)
    두지 말 것. 가능하면 **TTS 호출 전에** `content_review`를 먼저 통과시켜서
    애초에 사후 재생성 자체가 필요 없게 만드는 게 최선이지만, 사후에 발견돼도
    내용 오류라면 재생성한다(길이 때문이 아니라 "틀려서" 고치는 것이므로 이
    금지 규칙과 무관).
...
- ⚠️ **권장 순서: `content_review`/`--lang-check` 통과를 먼저 끝내고 나서 TTS를
  호출한다** — 위 "콘텐츠 톤" 절의 TTS 규칙과 짝: 미리 리뷰해두면 사후
  재생성 자체가 필요 없어진다. 그래도 TTS 이후에 실제 오류가 발견되면
  재생성한다(길이 때문이 아니라 내용이 틀려서 고치는 건 금지 대상이 아님) —
  순서는 "웬만하면 이렇게"이지 절대 규칙은 아니다.
