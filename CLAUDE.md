...
- **선택**: `lib/rebuild_video.py`의 `select_format(topic)`이 topic 문자열
  ...
  실행하면 됨.
- ⚠️ **판서형 3:1:1 가중치(2026-08-08)** — "판서형이 반응 더 좋다"는 판단으로
  균등 분배(1:1:1)에서 판서형:`before_after_transition`:`checklist` = 3:1:1로
  변경(`_FORMAT_WEIGHTED_POOL`). **프로스펙티브만**(신규/재조립하는 것부터,
  기존 영상 소급 재조립 안 함 — [광고] 태그 롤아웃 때와 동일 원칙).
