...
- ⚠️ **영상에 "[광고]" 표시 필요 — 미구현, 구현 전까지 기존 topic은 광고/링크
  요소 전부 제거**(2026-08-05, 공정거래위원회 공식 지침 확인 — 상세는 archive
  "영상 광고 표시 의무" 절):
  ...
  - 아직 이 영상 오버레이가 구현 안 됨 — **구현되기 전까지, 실제로 그 숏폼
    영상(mp4)이 그대로 올라가는 플랫폼만** `suppress_product_block: true`로
    커미션 요소(링크·고지문구)를 뺀다: **유튜브 쇼츠·인스타그램 릴스·네이버
    클립·틱톡·페이스북**(2026-08-06 확인 — 페이스북도 `type:"text"` 태그와
    무관하게 실제로 쇼츠 영상이 첨부됨, 처음에 `type` 필드만 보고 텍스트
    전용으로 오판해서 한 번 잘못 뺐다가 정정한 적 있음). ⚠️ **텍스트/이미지만
    올라가는 플랫폼(네이버 블로그·티스토리·쓰레드·인스타그램 카드뉴스)은
    이 문제와 무관 — 절대 `suppress_product_block`을 걸지 말 것**, 쓰레드는
    영상 대신 이미지 1장만 첨부하는 게 설계상 확정된 방식이라 여기 포함 안
    됨. 새 topic 작성 시에도 이 기준(실제 영상 첨부 여부)으로 판단할 것 —
    `type` 필드 값을 곧이곧대로 믿지 말고 실제 업로드 흐름을 확인.
  - **오버레이 구현 후 플랫폼별 대응(2026-08-06 확정)** — "[광고]" 태그가
    실제로 붙는다고 전부 일괄로 링크를 되살리는 게 아니라 플랫폼마다 다르게
    간다:
    - **유튜브 쇼츠·인스타그램 릴스(한국어+글로벌 전부)**: 오버레이가
      생겨도 **계속 보류** — 채널 구독자·팔로워 수가 아직 적어서 판매
      유도보다 성장이 우선이라는 별개의 비즈니스 판단(공정위 이슈와
      무관). "나중에 세팅 완료되고 나면" 다시 켤 예정, 그 세팅이 뭔지는
      아직 미정 — 다시 켜기 전에 반드시 사용자 확인.
    - **네이버 클립**: "[광고]" 태그 들어가는 즉시 네이버 쇼핑 커넥트
      연동 재개(사용자 확정) — 이미 만들어둔 클립 앱 네이티브 "상품" 버튼
      플로우를 그대로 씀(위 `network` 절 참고). 표시 규정(정확한 출처는
      못 찾음 — 실제 활동자들이 정리한 3차 가이드 기준, 공식 문서 아님을
      감안할 것):
      ...
      2. **대가성 문구는 캡션 본문이 아니라 "첫 번째 고정 댓글"로 별도
         ...
         말 것. 오버레이 구현 시 대시보드 dock에 이 댓글 문구를 복사하는
         버튼을 추가할 것(쓰레드·페이스북의 "댓글용 링크 텍스트 복사"
         버튼과 같은 패턴 재사용 가능).
    - **페이스북**: 오버레이 구현되면 다시 켤지 여부는 아직 미정 — 그
      시점에 사용자에게 확인.
- **필수 플래그**(한국어 topic, 빠짐없이):
  ...
  - `suppress_product_block: true` — 위 "[광고]" 항목 때문에 실제 숏폼 영상이
    첨부되는 플랫폼(유튜브 쇼츠·인스타그램 릴스·네이버 클립·틱톡·페이스북)에만
    건다(구현 전까지) — 네이버 블로그·티스토리·쓰레드·인스타 카드뉴스처럼
    영상이 안 붙는 플랫폼엔 걸지 말 것
