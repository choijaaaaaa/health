...
## 카드뉴스 단일 채널 — 네이버 블로그만 (2026-09-09 확정)

**영상 트랙은 다시 중단, 상품 링크는 네이버 커넥트만 쓴다.** 2026-09-01에 재개했던
영상 트랙을 일주일 만에 다시 접었다("이랬다저랬다하긴 하는데 진짜 필요없을듯") —
카드뉴스 하나로만 간다. 쿠팡 파트너스 링크 기능도 함께 제거했다(네이버 블로그만
남으니 쿠팡 링크는 애초에 어느 캡션에도 반영되지 않는 죽은 입력이었음).

- `lib/dashboard.py`/`lib/mission_control_sync.py`의 플랫폼 필터는 이제 **"네이버
  블로그" 하나만 허용하는 화이트리스트**(`_ALLOWED_PLATFORMS`)다 — 유튜브 쇼츠·
  네이버 클립·페이스북·인스타그램 릴스·틱톡·쓰레드는 전부 대시보드·mission-control
  양쪽에서 숨는다.
- ⚠️ **원본 데이터는 안 지웠다** — 108편 렌더된 mp4, 151개 topic에 복원해둔 페이스북
  캡션 등은 로컬/git에 그대로 남아있고 표시만 꺼둔 상태다(세 번째 번복에 대비한 가역적
  처리 — 되살리려면 `_ALLOWED_PLATFORMS`만 넓히면 됨).
- 상품 링크 UI(`_dock_products()`/`_product_links_bottom_section()`, mission-control
  `ProductLinkDock`/`MissingLinksPanel`/`apply-product-links.ts`)에서 쿠팡 검색·입력·
  검증·고지문을 전부 제거 — 네이버 브랜드커넥트 링크만 남는다.
- 새 topic은 `platform_captions.json`에 **네이버 블로그 하나만** 넣을 것 — 아래
  "카드뉴스 단일 트랙" 절 규칙 그대로.
- 영상 트랙 상세 규칙(렌더 명령어·포맷 다각화·TTS·모션 등)은 `CLAUDE_ARCHIVE.md`
  "영상 트랙 절"에 그대로 남아있다 — 재개하게 되면 거기서 찾을 것.
