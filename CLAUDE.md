...
기본값). ⚠️ **업로드 플랫폼(2026-08-26 갱신)** — 카드뉴스는 **네이버 블로그**에
올린다. 인스타그램·페이스북은 채널 자체가 폐지되어(아래 절) 더 이상 넣지 않는다.
`platform_captions.json`엔 네이버 블로그만 넣고, 영상 필요 플랫폼(유튜브쇼츠·
틱톡·네이버클립)과 쓰레드는 넣지 않는다. `narration.txt`도 만들지 않는다.

## ⚠️ 인스타그램·페이스북 채널 폐지 (2026-08-26)

**카드뉴스 제작은 그대로 유지하고, 발행 채널에서 인스타그램·페이스북만 뺀다.**
`lib/card_news.py`·`card_news_hub.py`·카드 이미지·`card_news_spec.json`은 전부
그대로다 — 없어진 건 채널뿐이다.

- 삭제: `lib/instagram_upload.py`, `lib/facebook_upload.py`, `lib/meta_posting_audit.py`
- `lib/meta_upload.py`는 Threads가 쓰므로 남기고, 인스타/페북 전용 접근자
  (`get_ig_id`/`get_page_id`/`get_page_access_token`)만 제거
- `video_assembler`의 `build_instagram_safe_video`(인스타 릴스 데스크탑 업로드용
  여백 버전) 제거 — 인스타에 안 올리므로 이 파생본이 필요 없다
- `platform_captions.json` 77개에서 인스타/페북 항목 148개, `social_accounts.json`
  에서 인스타그램 릴스·카드뉴스·페이스북 제거
...
- **로테이션(2026-08-25 개편)**: 브런치·핀터레스트는 항상 제외. 한국어 topic은
  **네이버 블로그**만(카드뉴스 단일 트랙 — 맨 위 절 참고, 인스타/페북은 2026-08-26
  채널 폐지). 아래 유튜브 쇼츠·틱톡·네이버 클립 관련 서술은 영상 트랙 재개 전까지
  비활성.
...
- ⚠️ **유튜브 쇼츠 로테이션 제외(2026-08-21, "트래픽이 전혀 나오지않아")** —
  숏츠 영상 자체(mp4)는 계속 만들어서 틱톡·네이버 클립엔 그대로 쓰지만, 유튜브 업로드용 캡션·자동 업로드는 더 이상 안 한다.
...
- ⚠️ **"[광고]" 표시 의무 — 화면 오버레이 부분은 영상 중단으로 비활성, 캡션
  ...
  본문에 링크를 넣는 쪽이므로 캡션 맨 아래 고지 한 덩어리로 처리한다(인스타·
  페북용 "댓글에 링크" 방식은 2026-08-26 채널 폐지로 해당 없음).
  ...
  - **플랫폼별 대응(2026-08-06 확정)** — 태그가 화면에 붙는다고 전부 일괄로
    ...
    - **네이버 클립도 재개(2026-08-06)**: `suppress_product_block: false`로
      ...
      - ⚠️ **"[광고] + 고지 문장"은 캡션 본문 맨 아래, 한 덩어리로(2026-08-10
        ...
        넣을 필요 없음.
  - 텍스트/이미지만 올라가는 플랫폼(네이버 블로그·쓰레드)은 이 문제와 무관 — 절대 `suppress_product_block`을 걸지 말 것,
    쓰레드는 영상 대신 이미지 1장만 첨부하는 게 설계상 확정된 방식이라 여기
    포함 안 됨.
- **필수 플래그**(한국어 topic, 빠짐없이):
  - `no_caption_link: true` — 틱톡(URL 클릭 안 되는 플랫폼)
  ...
  - `suppress_product_block: true` — 유튜브 쇼츠·틱톡엔
    계속 건다(위 "[광고]" 항목 참고). **네이버 클립
    (2026-08-06)은 안 건다**(`false` 또는 필드 자체를 생략) — 재개. 네이버
    블로그·쓰레드처럼 영상이 안 붙는 플랫폼엔
    애초에 걸지 말 것.
  - `link_in_comment: true` — 쓰레드(본문 링크 대신 댓글에), 항상
...
- 언어별 리서치 체크리스트는 `data/global_research_rules.md`(1차 타겟 국가,
  ...
  `data/global_channels.json`(2026-08-26 인스타 채널 폐지로 `instagram_url`
  제거됨 — 언어별 유튜브 채널·OAuth 상태만 남음).
...
손대지 말 것). 영상 자체(숏츠 mp4)는 계속 만들어서 틱톡·
