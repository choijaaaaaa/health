...
- ⚠️ **영상 우상단 "[광고]" 오버레이 — 구현 완료, 2026-08-06부터 신규 topic
  ...
  - **플랫폼별 대응(2026-08-06 확정)** — 태그가 화면에 붙는다고 전부 일괄로
    ...
    - **네이버 클립도 재개(2026-08-06)**: `suppress_product_block: false`로
      ...
      (위 `network` 절 참고). 표시 규정(정확한 출처는 못 찾음 — 실제
      활동자들이 정리한 3차 가이드 기준, 공식 문서 아님을 감안할 것):
      1. **"[광고]" 텍스트는 캡션 맨 첫머리**(쿠팡 파트너스처럼 본문
         중간/상단이 아니라 진짜 첫 줄) — 새 topic은 처음부터 이렇게 쓸 것.
      2. **대가성 문구는 캡션 본문이 아니라 "첫 번째 고정 댓글"로 별도
         작성** — "이 포스팅은 네이버 쇼핑 커넥트 활동의 일환으로, 판매
         발생 시 수수료를 제공받습니다"를 영상 업로드 직후 댓글로 달아야
         함. 쿠팡 고지문구(본문에 위치)와 위치 규칙이 다르니 섞어 쓰지
         말 것. `dashboard.py`에 이미 이 문구를 복사하는 버튼
         (`.copy-naver-connect-comment`, "📌 커넥트 댓글 문구 복사")이
         구현돼 있음 — 네이버 클립 카드의 `suppress_product_block`이
         false면 자동으로 뜬다.
