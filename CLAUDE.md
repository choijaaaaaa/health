...
- **비한국어 topic의 TTS는 topic당 1회만 호출한다** — 45초 초과해도(1분
  넘어도) narration.txt를 다시 써서 재생성하지 않는다(2026-08-05, "돈 존나
  나간다" — TTS는 글자수 과금이라 "뽑고→길면 재작성→다시 뽑고" 반복이 API
  호출 배수로 이어짐). 대신 `lib/typecast_tts.py`의 `estimate_duration(text,
  lang)`으로 **작성 전에** 예상 길이를 가늠해서 애초에 적당한 분량으로 쓴다
