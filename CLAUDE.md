...
⚠️ **위 절들의 상세 내용은 2026-08-31부로 `CLAUDE_ARCHIVE.md`의 "영상 트랙 절"로
옮겼다** — 본문이 1,600줄까지 불어나 지금 지켜야 할 규칙이 묻혀서다. 삭제가 아니라
이관이니 영상 트랙을 재개하면 거기서 그대로 되살릴 것(영상 조립·BGM·포맷 다각화·
TTS 3종·유튜브 업로드·캐릭터 모션).

"새 topic 해줘"라는 요청도 이제 자동으로 **"카드뉴스만"** 모드로 처리한다(아래
"두 트랙" 절 참고 — 예전엔 요청 문구로 선택했지만 이제 카드뉴스가 유일한
기본값). ⚠️ **업로드 플랫폼(2026-08-26 갱신)** — 카드뉴스는 **네이버 블로그**
한 곳에만 올린다. 인스타그램·페이스북 카드뉴스는 폐지(캡션 673개 제거) —
**영상 채널로서의 인스타그램 릴스·페이스북은 그대로 살아있다**, 없어진 건
"카드뉴스를 올리는 곳"뿐이다. `platform_captions.json`엔 네이버 블로그만 넣고,
영상 필요 플랫폼(유튜브쇼츠·틱톡·인스타릴스·네이버클립)과 쓰레드는 넣지 않는다.
`narration.txt`도 만들지 않는다.
