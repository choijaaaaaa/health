...
- **완전 자동화 모드** — "다음 topic 해줘"류 요청은 소재 선정→리서치→콘텐츠 작성→
  ...
  - ⚠️ **"끝까지"는 한국어(ko) 하나로 끝나는 게 아니라 글로벌 6개 언어(ko/en/
    ...
    세션 양쪽에서 실제 발생). ko만 먼저 끝내고 나머지 5개 언어를 별도 단계로
    미루는 것 자체는 괜찮음(리서치 규모상 자연스러움) — 다만 **그 상태를
    "작업 완료"로 보고하지 말고, 이어서 계속 en/es/ja/pt/ru 리서치·대본·TTS·
    영상까지 진행할 것**. topic 하나의 진짜 완료 기준은 6개 언어 전부 +
    영상까지.
...
- **topic 폴더명** = 핵심 카테고리 단어 + `_N`(예: `가슴쓰림_1`). 서술형 접미사
  ...
  - ⚠️ **카테고리 단어는 실제 신체 부위·증상을 가리키는 가장 평이한 단어를
    ...
    눈·입·귀·코·손·머리·냄새·피부·소화·순환·근골격·수면·뇌·비뇨기·어지럼증·
    혈당·여성. 새 topic이 이 중 하나에 해당하면 새 카테고리를 만들지 말고
    여기 번호를 이어서 붙일 것. 서로 다른 장기라 하나의 신체부위명으로 못
    묶는 주제(소화/순환/근골격/비뇨기 등)만 예외적으로 계통명을 쓴다.
...
- ⚠️ **인스타그램 릴스용 안전 여백 영상 — 포맷 무관 전부 필수**: 칠판 포맷만의
  ...
  `checklist`) 전부, ko/en/ja/es/pt(러시아어 제외, 아래
  "플랫폼 캡션" 절 참고) 전 언어에 빠짐없이 만들어야 한다.
...
- **로테이션**: 브런치·핀터레스트·인스타그램 카드뉴스는 항상 제외. 한국어
  ...
  두 항목만**(유튜브는 자동 업로드라 대시보드 UI에서 제외). 러시아어(ru)는
  예외로 Instagram Reels 없이 YouTube Shorts만(인스타 계정 수 한계) — 인스타용
  크롭 영상(`build_instagram_safe_video`)도 ru topic엔 만들지 말 것.
...
중단 — 채널 자체를 없앨지는 사용자가 별도로 판단(2026-08-09 기준 미정, Claude
Code가 채널·업로드 영상을 직접 삭제하지는 않음). 새 topic·언어는 이 3개 범위
안에서만 진행할 것 — 확장이 다시 필요해지면 git 히스토리에서 복원.
