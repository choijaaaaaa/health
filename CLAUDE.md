...
- ⚠️ **소재 선정(2026-08-10)** — "다음 topic 해줘"처럼 소재가 정해지지 않은
  ...
  1. `python3 -m lib.trend_check --show`(오늘자 스냅샷 없으면 `--daily`)로
     `data/seasonal_topics.md` 후보들의 구글 트렌드 모멘텀 확인
  2. 이번 달 계절 절 + 모멘텀(▲) 겹치는 카테고리 우선, 기존 topic과
     `ls data/ | grep "^<카테고리>_"`로 중복 확인
  3. 네이버 쪽 트렌드(데이터랩·뉴스 API)는 보류 중(사용자가 해외 체류로
     네이버클라우드플랫폼 휴대폰 인증 불가 — 한국 복귀 후 이어감) —
     그때까지 WebSearch로 정성적 보완만
  4. 급성 유행 이슈(감염병 등)는 계절 캘린더보다 우선하되, 반드시 공식
