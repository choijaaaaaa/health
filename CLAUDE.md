...
- 네이버 블로그·티스토리는 같은 원고 금지(도입부·설명 문장을 다르게 재작성,
  번호/이미지 순서는 유지). ⚠️ **첫 줄(제목)도 서로 달라야 함(2026-08-12)** —
  `test_content_rules.py`의 `test_naver_blog_and_tistory_not_duplicated`가 캡션
  첫 non-empty 줄과 본문 전체 둘 다 자동 검사한다(본문만 다르고 첫 줄인 제목이
  같으면 실패). 네이버 블로그는 `platform_captions.json`의 `title` 필드를 그대로
  첫 줄에 쓰고, 티스토리는 같은 내용이라도 문구 자체를 다르게 새로 쓸 것.
