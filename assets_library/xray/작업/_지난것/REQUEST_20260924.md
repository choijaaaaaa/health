...
행위·기전 클립 13종. **미드저니로 스틸을 뽑고 → Flow에 start 프레임 단독으로 넣고 → 모션 프롬프트**를 붙인다.
결과는 `assets_library/xray/RENDER/<이름>.mp4`로 저장하면 세션이 정리한다.

🚨 Flow는 **start 프레임만** 준다(end를 주면 Veo가 사이를 맞추려고 피사체를 변형시킨다).
🚨 길이는 4·6·8·10초 네 가지뿐. 아래는 전부 **6초** 기준이다.
...
## 3. `act_nausea_push_plate_away` — 대사_14
...
## 4. `act_epigastric_bore_to_back` — 대사_14
...
## 5. `act_urgent_watery_bowel` — 비뇨기_16
...
## 6. `m_crystal_precipitation` — 소화_13
...
## 8. `act_shared_pot_dipping` — 소화_18
...
## 11. `act_toilet_strain_faint` — 순환_12
