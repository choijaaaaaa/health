...
_JARGON = {
    "미주신경": "신경", "알룰로스": ("설탕", "단맛"), "에리스리톨": ("설탕", "단맛"), "사구체여과율": "신장", "크레아티닌": ("노폐물", "찌꺼기"), "하시모토": "면역",
...
# 묶은 건 과잉이었다.
# 🚨 2026-09-24 추가: "~부터 보세요/확인하세요"류 **지시형 꼬리**도 금지한다.
# 사용자 지시: "이 습관부터 보세요 이런거말고 이런 게 문제일 수 있어요 로 가자".
# 지시형은 시청자에게 숙제를 주고 끝나서 "그래서 뭔데"가 안 생긴다 — 대신 문제를
# 먼저 제시해 다음 문장을 듣게 만든다("~이런 게 문제일 수 있어요").
BANNED_HOOK_TAIL = re.compile(
    r"(?:다|라)면[^.!?]{0,20}?"
    r"(?:주목|저장부터|저장해|저장하세요|놓치지\s*마세요)"
    r"|(?:이것|이\s*습관|이\s*세\s*가지|이\s*네\s*가지|먹는\s*것)부터\s*(?:보|확인)"
)
