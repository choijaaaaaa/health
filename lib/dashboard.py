...
function applyProductLinks() {{
  document.querySelectorAll(".caption-box").forEach(box => {{
    ...
    // WHY linkInComment도 suppressBlock처럼 취급하는지(2026-08-04, "이건 굳이
    // 두번 넣을 필요가 없잖아... 그리고 아예 이 멘트 자체를 없애도 될거같은데?
    // 댓글에 넣으려면 넣고 안넣으려면 안넣고 그런거지 / 쿠팡 파트너스 활동 관련
    // 코멘트도 댓글에다 링크 남길 때 같이 올리면되는거아니여?"): 쓰레드·페이스북은
    // 실제 링크를 댓글에 다는지 여부가 매번 게시자 재량인 수동 흐름이라, 캡션
    // 본문에 "댓글에 남겨둘게요"라는 확정된 약속(+고지문구)을 위아래 두 번
    // 자동으로 박아넣는 게 맞지 않았다 — 이미 있는 "💬 댓글용 링크 텍스트 복사"
    // 버튼(COMMENT_LINK_INTRO + _buildLinkBlock(false), 고지문구 포함)이 실제로
    // 댓글에 달 때 쓸 텍스트를 그대로 제공하므로, 캡션 본문에는 아무것도 안
    // 붙이는 게 맞다.
    const suppressBlock = card.dataset.suppressProductBlock === "1" || linkInComment;
    const result = suppressBlock ? null : (noCaptionLink ? _buildCtaBlock(hasCommentDm) : _buildLinkBlock(hasNaverButton));
    ...
    // WHY 고지문을 맨 위(제목 다음)·맨 아래(캡션 최종 끝) 두 곳에 각각 한 번씩만
    // 넣는지(2026-08-03 재작업 — 위 _buildLinkBlock/_buildCtaBlock 주석 참고):
    // 공정위 지침상 "게시물 첫 부분"에 있어야 해서 제목 다음에 하나 넣고,
    // "스크롤 안 해도 위에서 보이지만 끝까지 읽은 사람도 다시 보게" 요청대로
    // 게시물 맨 끝에도 하나 더 넣는다 — 상품 블록 바로 위·아래에 몰아넣는 게
    // 아니라 게시물 전체를 고지문으로 감싸는 것이 실제 요구사항이었다.
    const topWrapped = AUTO_LINKS_START + result.disclosure + "\\n\\n" + blockText + AUTO_LINKS_END;
