...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  /* WHY 고정 뷰포트 높이 + 자체 스크롤(기본 iframe scrolling)을 유지하는지
     (2026-08-04, 실제 버그로 발견됨 — "위젯은 위아래로 이동하면서 따라오지
     않고, 안쪽으로 들어와서 다른 스크립트들을 가리게 되었어"): 예전엔 iframe
     높이를 내용물 실측 높이로 늘려서(load 시 scrollHeight 적용) 이 페이지
     전체가 하나의 스크롤처럼 보이게 했는데, 그러면 iframe 내부 문서 입장에서는
     "뷰포트 = 전체 문서 높이"라 스크롤이 아예 안 생겨서 언어별 dashboard.html
     안의 상품 링크 dock(position: fixed)이 스크롤을 전혀 못 따라오는 상태가
     됐다 — position:fixed/sticky는 항상 자기 문서 자신의 뷰포트 기준으로만
     동작해서, 부모 페이지 스크롤과는 애초에 연동될 수 없다(postMessage로
     직접 동기화하지 않는 한). 이중 스크롤바가 생기더라도 iframe이 실제로
     스크롤 가능한 뷰포트를 가져야 그 안의 fixed dock이 정상 동작하므로,
     고정 높이로 되돌리고 자동 리사이즈는 뺐다 — standalone으로 열었을 때와
     동일한 동작을 보장하는 쪽이 우선. */
  iframe.dash-frame {{ width: 100%; height: 82vh; border: 0; border-radius: 16px; background: var(--panel); }}
