...
function _buildCtaBlock(hasCommentDm, linkInComment) {{
  ...
  if (linkInComment) {{
    return _withDisclosure("🔗 구매 링크는 댓글에 남겨둘게요!", COUPANG_DISCLOSURE);
  }}
  if (!hasCommentDm) {{
    return _withDisclosure("🔗 상품 링크는 프로필에서 확인해주세요!", COUPANG_DISCLOSURE);
  }}
  ...
  return _withDisclosure(cta, COUPANG_DISCLOSURE);
}}
