...
function _buildCtaBlock(hasCommentDm, linkInComment) {{
  ...
  if (linkInComment) {{
    return {{text: "🔗 구매 링크는 댓글에 남겨둘게요!", disclosure: COUPANG_DISCLOSURE}};
  }}
  if (!hasCommentDm) {{
...
function applyProductLinks() {{
  document.querySelectorAll(".caption-box").forEach(box => {{
    ...
    const result = suppressBlock ? null : (noCaptionLink ? _buildCtaBlock(hasCommentDm, linkInComment) : _buildLinkBlock(hasNaverButton));
