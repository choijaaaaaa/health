...
# WHY: 파이프라인/QA 스크립트가 실행 중 발견한 문제(영상 스테일니스, 업로드
# 실패 등)를 여러 프로젝트가 각자 콘솔에만 찍고 흩어놓는 대신 한곳(/issues
# 페이지)에서 모아볼 수 있게 mission-control의 POST /api/ingest/log로 전달한다.
#
# ⚠️ 루트 CLAUDE.md "무음 실패 금지" 원칙의 의도적 예외: report_issue()는
# 절대 예외를 밖으로 던지지 않는다 — 환경변수 미설정(로컬에 mission-control을
# 안 띄워둔 세션이 대부분)이나 네트워크 실패가 실제 콘텐츠 파이프라인(영상
# 조립·유튜브 업로드 등)을 중단시키면 안 되기 때문. 이 파일은 부가 로깅
# 채널일 뿐 — 실패해도 호출부의 본 작업 흐름에는 아무 영향이 없어야 한다.
