# 나레이션 텍스트 TTS 오독 위험 검사(2026-08-18 도입, jp-review-shorts에서
# 먼저 확정된 규칙 이식). WHY: 콜라겐씨젤리_JP_1 나레이션 "미용 성분이
# 7종에서 10종으로"가 TTS에서 "칠곱종"/"셥종"처럼 한자어(칠·십)와
# 순우리말(일곱·열) 숫자 읽기가 뒤섞여 깨지는 사고가 실제로 발생했다
# (사용자 청취로 발견) — "N+순우리말 단위 카운터"(개/명/종/가지/번/살/
# 마리 등) 조합은 순우리말 숫자(하나~아흔아홉)로 읽는 게 자연스러운데,
# 원문에 아라비아 숫자를 그대로 써두면 TTS가 어느 쪽으로 읽을지 헷갈려한다.
# 이 프로젝트의 기존 "소수점 퍼센트는 말로 풀어 쓸 것" 규칙(lib/fish_tts.py
# 관련 CLAUDE.md 절)과 같은 문제의식 — 화면 표시용 텍스트(캡션 등)는
# 대상 아님, TTS가 실제로 읽는 narration.txt만 검사한다.
#
# 사용법: python3 -m lib.narration_qa data/<topic>/[<lang>/]narration.txt
